#tag Class
Protected Class FTCUndoBackspace
Inherits FTCUndoBase
	#tag Method, Flags = &h0
		Sub constructor(parent as FTCUndoManager, offset as FTInsertionOffset)
		  ...
		  ' Initialize the base.
		  Super.Constructor(parent, FTCStrings.BACKSPACE)
		...
		Sub undo()
		  ...
		  for i = count downto 0
		    ...
		    if item isa FTParagraph then
		      ...
		      ' Create a clone.
		      insertionItem = FTObject(item).clone
		      
		      ' Is the item a picture?
		      if insertionItem isa FTPicture then
		        
		        ' Reset the resize mode.
		        FTPicture(insertionItem).setResizeMode(false)
		        
		      end if
		      
		      ' Put the item back.
		      doc.insertObject(insertionItem)
		      
		      ' Is this a picture?
		      if item isa FTPicture then
		        
		        ' Was the picture in resize mode?
		        if FTPicture(insertionItem).isResizeMode then
		          
		          ' Get the current paragraph.
		          p = doc.getInsertionPoint.getCurrentParagraph
		          
		          ' Set the picture as the resize picture.
		          doc.setResizePicture(FTPicture(insertionItem), p)
		          
		          ' Turn off the insertion caret.
